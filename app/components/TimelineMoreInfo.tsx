...
const TimelineMoreInfo: React.FC<TimelineMoreInfoProps> = ({ data, setShowModal }) => {
  ...
  return (
    <motion.div
      ...
      animate='visible'
      exit='exit'
      className={`z-[20] w-[70rem] h-fit p-8 rounded-lg ${data.type === 'school' ? 'bg-blue-500' : data.type === 'work' ? 'bg-blue-700' : 'bg-cyan-900'}`}
    >
      <div className='flex items-center justify-between'>
        <div className='relative text-white text-3xl'>
            {data.type === 'school' ?  <IoSchoolSharp /> : data.type === 'work' ? <MdOutlineWork /> : <FaCode />}
        </div>
        <GrClose 
        ...
        {next ? (
          <ResumePictures setNext={setNext} picsArray={data.pics ? data.pics : []} />
        ) : (
          <>
              ...
              {data.bullets ? (
                <ul>
                  {data.bullets.map((bullet: string, i: number): ReactElement => {
                    return <li key={i} className='mt-4 text-lg'>{bullet}</li>;
                  })}
                </ul>
              ) : (
                <div className='gap-4'>
