              ...
              {i === current ? (
                <>
                  ...
                  <div className="group relative cursor-pointer items-center justify-center overflow-hidden transition-shadow hover:shadow-black/30">
                    <div className="absolute inset-0 bg-gradient-to-b group-hover:from-sky-800/70 group-hover:via-sky-800/60 group-hover:to-sky-800/70"></div>
                    <img 
                      src={pic.path} 
                      alt={pic.title}
                      className='object-scale-down w-[400px] h-[200px] sm:w-[800px] sm:h-[600px]' 
                    /> 
                    <div className="absolute invisible sm:visible inset-0 flex text-white translate-y-[70%] gap-6 flex-col items-center justify-center text-center transition-all duration-500 group-hover:translate-y-0">
                      <p className="text-[10px] sm:text-3xl font-bold mb-16 sm:mb-0">{pic.title}</p>
                      <p className="text-[10px] sm:text-2xl font-bold mb-16 sm:mb-0">{pic.time}</p>
                      <p className="text-[10px] sm:text-xl opacity-0 transition-opacity duration-300 group-hover:opacity-100 font-semibold">{pic.text}</p>
                    </div> 
                  </div>
                </>
              ) : <></>}
